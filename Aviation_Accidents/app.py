...
Departure=Base.classes.Departure_data
recent_accidents=Base.classes.Recent_data
...
@app.route('/Crash_locations/')
def crash_summ():
    return render_template('Crash_locations.html')
...
@app.route("/accident_summ")
def get_accidents_summ():
        results = db.session.query(Hist_summary.Year,Hist_summary.Total_Accidents,Hist_summary.Total_Fatalities).all()
        df=pd.DataFrame(results,columns=['Year','Total_accidents','Total_fatalities'])
        trace1 = {
                    "x": df["Year"].values.tolist(),
                    "y": df["Total_accidents"].values.tolist(),
                    "type": 'scatter',
                    "mode":'lines+markers',
        ...
        trace2 = {
                    "x": df["Year"].values.tolist(),
                    "y": df["Total_fatalities"].values.tolist(),
                    "type": 'scatter',
                    "mode":'lines+markers',
...
@app.route("/depart_summ")
def get_departure_summ():
        results = db.session.query(Departure.Year,Departure.Value).all()
        df=pd.DataFrame(results,columns=['Year','Total_Flights'])
        trace1 = {
                    "x": df["Year"].values.astype(str).tolist(),
                    "y": df["Total_Flights"].values.tolist(),
                    "mode": 'lines+markers'
        }
        data=[trace1]
        print(data)
